...
//            driver.quit();
    ...
    public void editPasswordOfSuperAdmin(String RoleIn, String SecondNameOBJ, String FirstNameOBJ,
        ...
        switch (RoleIn) {
            case ("SuperAdmin"):
                p.auth(p.LoginAUT_SA, p.PasswordAUT_SA);
//               //                p.moveToProfileUserIDChange();
...
//                p.waitTextPresent(p.userinfoname, SecondNameOBJ + " " + FirstNameOBJ + " " + MiddleNameOBJ);
            ...
            case ("Admin"):
                p.auth(p.LoginAUT_A, p.PasswordAUT_A);
            ...
            case ("Spec"):
                p.auth(p.LoginAUT_S, p.PasswordAUT_S);
                //                p.moveToProfileUserID(UserID);
                p.moveToProfileUserID(UserID);
                p.sleep(5000);
                try {
...
//            p.click(p.buttonFurtherYandex);
    ...
    public void findLetterYandex(String LoginIN, String Password, String Email, String SecondNameIN,
        ...
        System.out.println("Количество символов в пароле: " + p.codeLetterYandex.getText().length() + " - " + p.codeLetterYandex.getText());
...
//        Assert.assertEquals(driver.getCurrentUrl(), ConfigProperties.getTestProperty("baseurl") + "#/loginAPI/" +
//        Email + "/enterCode/" + Code);
    }
...
//            p.sleep(1000);
//            System.out.println("Ввели логин, нажимаем кнопку Далее");
//            p.click(p.buttonFurtherGoogleMail);
//            System.out.println("Вводим пароль");
            p.waitVisibility(p.passwordFormGoogleMail);
            p.clickAndSendKeysAndEnter(p.passwordFormGoogleMail, Password);
//            System.out.println("Ввели пароль, нажимаем кнопку Далее");
//            p.click(p.buttonFurtherGoogleMail);
            p.sleep(8000);
    ...
    public void findLetterGoogleMail(String LoginIN, String PasswordIN, String Email, String NewPasswordIN) {
        ...
        p.auth(LoginIN, NewPasswordIN);
        p.waitVisibility(p.userinfoname);
        Assert.assertEquals(ConfigProperties.getTestProperty("baseurl") + "/#/profile/summary", driver.getCurrentUrl());
    }

    public void editPasswordOfUserHighLevelOrg(String RoleIN, String LoginIN, String PasswordIN, String SecondNameOBJ
            , String FirstNameOBJ, String MiddleNameOBJ, String LoginOBJ, String NewPasswordOBJ, String UserID) {
        p.logout();
        switch (RoleIN) {
            case ("SuperAdmin"):
                p.auth(LoginIN, PasswordIN);
