...
public class MyProfilePage extends GlobalPage {

    MyProfilePage(RemoteWebDriver driver) {
    ...
    public void deleteUserAPI(String Token, String UserID1, String Response) {
    ...
    public void editUserPasswordAPI(String Token, String UserID, String Password, String Response) {
        ...
        Response response =
                given().log().all().
                        ...
                        contentType(ContentType.JSON).
                        body(jsonAsMapProfile).
                        baseUri(ConfigProperties.getTestProperty("baseurl")).basePath("/passport/admin/user/password").
                        when().post().
                        then().extract().response();

        System.out.println("Ответ при изменении пароля юзера: " + response.getBody().asString());
        Assert.assertTrue(response.getBody().asString().contains(Response));
    }
