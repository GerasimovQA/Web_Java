...
public class EditOrganizationPage extends GlobalPage {
    ...
    public EditOrganizationPage(RemoteWebDriver driver) {
    ...
    @FindBy(xpath = "//div[@class=\"info-block__title\"]/span[contains (text(),\"Оказываемые услуги \")]/../../../../../../../div/button/span[contains(text(),\"Изменить\")]")
    ...
    @FindBy(xpath = "//div[@class=\"dep-edit-map\"]/following-sibling::button/span[text()=\"Сохранить\"]")
    public WebElement buttonSaveAdressOrganization;

    // Кнопка "Отмена" сохранения адреса подразделения
    @FindBy(xpath = "//div[@class=\"dep-edit-map\"]/following-sibling::button/span[text()=\"Отмена\"]")
    public WebElement buttonDoNotSaveAdressOrganization;

    // Сайт организации
    @FindBy(xpath = "//div[contains (text(),\"Сайт\")]/following-sibling::div[1]")
    public WebElement siteOrganization;
    ...
    @FindBy(xpath = "//div[contains (text(),\"Другое\")]")
    public WebElement otherNameOrganization;
...
//                System.out.println(checkServices2.get(0).getAttribute("class") + " = " + Checkeds + " or " + CheckedAndFocus);
...
//                Assert.assertEquals(checkServices2.get(0).getAttribute("class"), Checkeds);
                ...
                System.out.println(listIconsDeleteSelectServicesAddUser.size() + " = " + listNameSelectServicesAddUser.size() +
                        " и " + (listNameSelectServicesAddUser.size()) + " = " + (textServices3.size() - 1) +
                        " и " + (textServices3.size() - 1) + " = " + (checkServices3.size() - 1));
                Assert.assertTrue(listIconsDeleteSelectServicesAddUser.size() == listNameSelectServicesAddUser.size() &
                        listNameSelectServicesAddUser.size() == (textServices3.size() - 1) &
                        (textServices3.size() - 1) == (checkServices3.size() - 1));
...
//                Assert.assertEquals(checkServices2.get(1).getAttribute("class"), Indeterminate);
                ...
                try {
                    for (int b = listSelectedCheckBoxInTree.size() - 1; b > -1; b--) {
                        ...
                        System.out.println(SelectedServices);
                        System.out.println("Сняли выделение с: " + listStringSelectedServicesInTree.get(b).getText().replace("\n", " "));
                        scrollWithOffset(listSelectedCheckBoxInTree.get(b), 0, 0);
//                        scrollWhell(0,-100);
            ...
            case "5.Услуги":
                ...
                for (WebElement check2 : checkServices2) {
                    System.out.println("Выделяем чек-бокс второго уровня");
                    scrollEndPage();
                    click(check2);
                }
                sleep(1000);
                for (WebElement Service : stringTreeServices3) {
                    ...
                    System.out.println(checkServices1.get(0).getAttribute("class") + " = " + Checkeds + " or " + CheckedAndFocus);
